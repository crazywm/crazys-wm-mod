		...
		if (DayNight == SHIFT_DAY)
		{
				...
				g_Girls.GetStat(current, STAT_HEALTH) >= 80 && g_Girls.GetStat(current, STAT_TIREDNESS) <= 20)
			{
					...
					&& current->m_PrevDayJob != 255 && current->m_PrevNightJob != 255)	// 255 = nothing, in other words no previous job stored
				{
					...
					MatronMsg += gettext("The Chairman puts ") + girlName + gettext(" back to work.\n");
 ...
 *		JOB PROCESSING
 */
		unsigned int restjob = JOB_CLINICREST;	// `J` added this to allow for easier copy/paste to other buildings
		u_int sw = 0;							//	Job type
		...
		if (sw >= JOB_GETHEALING && sw <= JOB_JANITOR)
		{
