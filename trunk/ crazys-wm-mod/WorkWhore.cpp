...
bool cJobManager::WorkWhore(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	...
	if(bStreetWork && g_Dice.percent(5))
	{
		...
		if(rival)
		{
			...
			{
				ss.str("");
				summary		+= girl->m_Realname + gettext(" was attacked by enemy goons. \n");
				//message		+= "She ran into some enemy goons and was attacked.\n";
				ss	<< girl->m_Realname << gettext(" ran into some enemy goons and was attacked.\n");
				...
				if (iNum > 0)
				{
					...
					ss	<< gettext("She fought back and was hurt taking ") << iNum << gettext(" points of damage.\n");

				}
				else
					ss	<< gettext("She fought back taking no damage.\n");
...
//			girl->m_Events.AddMessage(message, IMGTYPE_PROFILE, DayNight);
	...
	for(int i=0; i < LoopCount ; i++)	// Go through all customers
	{
		...
		if(acceptsGirl == false)	
		{
			...
			{
				fuckMessage += gettext("Customer sleeps with her because they are very horny.\n");
				acceptsGirl = true;
			}
			else if(((g_Girls.GetStat(girl, STAT_BEAUTY)+g_Girls.GetStat(girl, STAT_CHARISMA))/2) >= 90)	// if she is drop dead gorgeous
			{
				fuckMessage += gettext("Customer sleeps with her because they are stunned by her beauty.\n");
				acceptsGirl = true;
			}
			else if(g_Girls.GetStat(girl, STAT_FAME) >= 80)	// if she is really famous
			{
				fuckMessage += gettext("Customer sleeps with her because she is so famous.\n");
				acceptsGirl = true;
			}
			...
			{
				fuckMessage += girl->m_Realname + gettext(" uses magic to get the customer to sleep with her.\n");
				g_Girls.UpdateStat(girl, STAT_MANA, -20);
				acceptsGirl = true;
			}
		...
		if(!bCustCanPay)
		{
			...
			if(g_Dice.percent(Cust.m_Stats[STAT_CONFIDENCE] - 25))	// Runner
			{
				...
				{
					if(g_Dice.percent(50))
						fuckMessage += gettext(" The customer couldn't pay and managed to elude your guards.");

					else
					{
						fuckMessage += gettext(" The customer couldn't pay and tried to run off. Your men caught him before he got out the door.");
					...
					fuckMessage += gettext(" The customer couldn't pay and ran off. There were no guards!");
			...
			else
			{
				...
				if(g_Dice.percent(g_Girls.GetStat(girl, STAT_INTELLIGENCE)))
				{
					...
					fuckMessage += gettext(" The customer couldn't pay the full amount, so your girl turned them over to your men.");
					SetGameFlag(FLAG_CUSTNOPAY);
				}
				else
					fuckMessage += gettext(" The customer couldn't pay the full amount.");
		...
		else if(g_Dice.percent((40 - Cust.m_Stats[STAT_HAPPINESS]) / 2) && g_Dice.percent(Cust.m_Stats[STAT_CONFIDENCE] - 25))
		{
			if(g_Gangs.GetGangOnMission(MISS_GUARDING))
			{
				if(g_Dice.percent(50))						
				{
					fuckMessage += gettext(" The customer refused to pay and managed to elude your guards.");
					pay = 0;
				}
				else
				{
					fuckMessage += gettext(" The customer refused to pay and tried to run off. Your men caught him before he got out the door and forced him to pay.");
					Cust.m_Money -= (unsigned) pay; // WD: ??? not needed Cust record is not saved when this fn ends!  Leave for now just in case ???
				}
			}
			else
			{
				fuckMessage += gettext(" The customer refused to pay and ran off. There were no guards!");
				pay = 0;
			}		
...
//					pay	+= (int)Cust.m_Money; // WD: Borked Cust.m_Money could have any amount and is +=
		...
		else  // Customer has enough money
		{
			...
			if((int)Cust.m_Money >= 20 && Cust.m_Stats[STAT_HAPPINESS] > 90)	
			{
				...
				ss.str("");
				ss << gettext(" Received a tip of ") << tip << gettext(" gold");
				fuckMessage += ss.str();
				...
				if(girl->is_slave())
				{
					fuckMessage += gettext(", which you claim");
				...
				if(Cust.m_Official == 1)
				{
					...
					fuckMessage += gettext(" It turns out that the customer was a government official, which lowers your suspicion.");
				}
			}
		}
	...
	if(!bStreetWork)  {
		if (g_Customers.GetNumCustomers() == 0) {
			girl->m_Events.AddMessage(gettext("No more customers."), IMGTYPE_PROFILE, DayNight);

		}
		else if(NumSleptWith < NumCusts) {
			girl->m_Events.AddMessage(girl->m_Realname + gettext(" ran out of customers who like her."), IMGTYPE_PROFILE, DayNight);
		}
	}
	...
	ss.str("");
	if(bStreetWork)
		ss << girl->m_Realname << gettext(" worked the streets and saw ") << NumSleptWith << gettext(" customers this shift.");

	else
		ss << girl->m_Realname << gettext(" saw ") << NumSleptWith << gettext(" customers this shift.");
