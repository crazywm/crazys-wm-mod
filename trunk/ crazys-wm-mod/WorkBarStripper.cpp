...
bool cJobManager::WorkBarStripper(sGirl* girl, sBrothel* brothel, bool Day0Night1, string& summary)
{
	...
	if (jobperformance >= 245)
	{
		...
		if (roll <= 20)
		{
			message += girlName + " use of the pole amazes the patrons. They truly have no idea how she does the things she does.\n";
			brothel->m_Happiness += 10;
			brothel->m_Fame += 5;
		}
		else if (roll <= 40)
		{
			message += girlName + " throws her bra into the crowd causing a fight to erupt over who got to take it home. They just love her.\n";
			brothel->m_Happiness += 10;
		}
		else if (roll <= 60)
		{
			message += "People push and shove to get to the front of the stage for " + girlName + "'s dance set.\n";
			brothel->m_Fame += 10;
		}
		else if (roll <= 80)
		{
			message += girlName + "'s smile is enough to bring in the tips. You sometimes wonder if she even needs to take her clothes off.\n";
			brothel->m_Happiness += 10;
			wages += 15;
		}
		else
	...
	else if (jobperformance >= 185)
	{
		...
		{
			message += girlName + " throws her bra into the crowd causing an uproar of hooting and screaming.  She knows how to drive them crazy.\n";
			brothel->m_Happiness += 10;
		}
		else if (roll <= 60)
		{
	...
	else if (jobperformance >= 145)
	{
		...
		{
			message += "Since you let " + girlName + " choose the music she dances to, her erotic dances have improved markedly.\n";
		}
		else
		{
	...
	else if (jobperformance >= 70)
	{
		...
		if (roll <= 20)
		{
			message += girlName + " spun around on the pole too much and got dizzy, causing her to fall off-stage buck-naked into the crowd. "
		...
		{
			message += girlName + " at least managed to get all her clothes off this time.\n";
		}
		else
		{
	...
	{
		message += girlName + " was stiff and nervous, constantly making mistakes. She really isn't good at this job.\n\n";
		wages -= 15;
		if (roll <= 20)
		{
	...
	{
		message += girlName + " tried to sell private dances ";
		if (roll < 5)
		{
			message += "and was able to sell a vip dance againts all odds.";
			wages += 50;
		}
		if (roll < 20)
		{
			message += "and was able to sell a lap dance.";
			wages += 25;
		}
		else
		{
			message += "but wasn't able to sell any.";
		}
	}
	else
	{
