...
bool cJobManager::WorkFilmSex(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	string message = "";
	...
	girl->m_Pay += 50;
	message = girlName;
	message += " worked as an actress filming sex scences.\n\n";
	
	int roll = g_Dice%100;

	if(roll <= 10 && g_Girls.DisobeyCheck(girl, ACTION_WORKMOVIE, brothel)) 
	{
		message = girlName + " refused to fuck on film today.\n";
	...
	else if(roll <= 15)
	{
		...
		message += girlName + " didn't want sex today, but needed the money.\n\n";
		jobperformance += -5;
	}
	else if(roll >=90)
	{
		g_Girls.UpdateEnjoyment(girl, ACTION_WORKMOVIE, +3, true);
		message += girlName + " loved having a man in her pussy today.\n\n";
		jobperformance += +5;
	}
	else
	{
	...
	jobperformance += 0; // Modifier for what kind of sex scene it is.. normal sex is the baseline at +0
	// remaining modifiers are in the AddScene function --PP
	string finalqual = g_Studios.AddScene(girl, SKILL_NORMALSEX, jobperformance);
