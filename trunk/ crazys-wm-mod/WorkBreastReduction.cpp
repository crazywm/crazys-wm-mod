...
bool cJobManager::WorkBreastReduction(sGirl* girl, sBrothel* brothel, int Day0Night1, string& summary)
{
	string message = "";
	int msgtype = Day0Night1;

	if (girl->m_YesterDayJob != JOB_BREASTREDUCTION)	// if she was not in surgery yesterday, 
	{
	...
	{
		message = girl->m_Realname + gettext(" already has Small Boobs so she was sent to the waiting room.");
	...
	if (girl->m_WorkingDay >= 5)
	{
		...
		if (g_Girls.HasTrait(girl, "Abnormally Large Boobs"))
		{
			...
			ss << "Her Abnormally Large Boobs trait is reduced to Big Boobs traits.\n";
		}
		else if (g_Girls.HasTrait(girl, "Big Boobs"))
		{
			g_Girls.RemoveTrait(girl, "Big Boobs");
			ss << "She no longer has Big Boobs.\n";
		}
		else if (!g_Girls.HasTrait(girl, "Small Boobs"))
		{
			girl->add_trait("Small Boobs", false);
			ss << "She now has Small Boobs.\n";
		}
