...
bool cJobManager::WorkGetFertility(sGirl* girl, sBrothel* brothel, int Day0Night1, string& summary)
{
	string message = "";
	...
	{
		message = girl->m_Realname + gettext(" is already Fertile so she was sent to the waiting room.");
		if (Day0Night1 == SHIFT_DAY)	girl->m_Events.AddMessage(message, IMGTYPE_PROFILE, EVENT_WARNING);
		girl->m_DayJob = girl->m_NightJob = JOB_CLINICREST;
		return false;	// not refusing
	}
