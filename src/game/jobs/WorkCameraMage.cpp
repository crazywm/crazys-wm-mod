...
bool WorkCameraMage(sGirl& girl, bool Day0Night1, cRng& rng)
{
    ...
    {
        ss << "The Director assigned ${name}to run the camera for the week";
    }
    ...
    if (!SkipDisobey)    // `J` skip the disobey check because it has already been done in the building flow
    {
        if (roll <= 10 && girl.disobey_check(actiontype, JOB_CAMERAMAGE))
        {
            ...
            {
                ss << " but ${name}";
                if (is_Actress_Job(girl.m_DayJob))    ss << " wanted to be in front of the camera rather than behind it.\n";
    ...
    double jobperformance = girl.job_performance(JOB_CAMERAMAGE, false);
...
double JP_CameraMage(const sGirl& girl, bool estimate)// not used
{
