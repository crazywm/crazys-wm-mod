...
bool cJobManager::WorkFilmTitty(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	string message = "";
	...
	girl->m_Pay += 60;
	message = girlName;
	message += (" worked as an actress filming titty fucking scenes.\n\n");
	
	int roll = g_Dice%100;

	if(roll <= 10 && g_Girls.DisobeyCheck(girl, ACTION_WORKMOVIE, brothel)) 
	{
		message = girlName + " refused to have her titty's fucked on film today.\n";
	...
	else if(roll <= 15)
	{
		...
		message += girlName + " used her breasts on his cock, but didn't like it.\n\n";
		jobperformance += -5;
	}
	else if(roll >=90)
	{
		g_Girls.UpdateEnjoyment(girl, ACTION_WORKMOVIE, +3, true);
		message += girlName + " loved having his cock slide bewteen her breasts.\n\n";
		jobperformance += +5;
	}
	else
	{
		g_Girls.UpdateEnjoyment(girl, ACTION_WORKMOVIE, +1, true);
		message += girlName + " had a pleasant day using her tits to get her co-star off.\n\n";
	}
	...
	string finalqual = g_Studios.AddScene(girl, SKILL_TITTYSEX, jobperformance);
	message += "Her scene us valued at: " + finalqual + " gold.\n";

	girl->m_Events.AddMessage(message, IMGTYPE_TITTY, DayNight);
