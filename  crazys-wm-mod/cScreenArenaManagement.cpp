...
void cScreenArenaManagement::check_events()
{
	...
	if(g_InterfaceEvents.CheckListbox(jobtypelist_id))
	{
		...
		else
		{
			...
			RefreshJobList();
			EditTextItem(g_Arena.m_JobManager.JobFilterDescription[selection], jobtypedesc_id);
		}
	}
	if(g_InterfaceEvents.CheckListbox(joblist_id))
	{
		...
		if(selection != -1)
		{
			...
			EditTextItem(g_Arena.m_JobManager.JobDescription[selection], jobdesc_id);
			...
			while(GSelection != -1)
			{
				...
				if(selected_girl)
				{
					...
					if(g_Arena.m_JobManager.HandleSpecialJobs(g_CurrArena, selected_girl, selection, old_job, day))
					{
					...
					if (old_job != selection)
					{
						...
						ss.str("");
						ss << g_Arena.m_JobManager.JobName[(int)selected_girl->m_DayJob];
						SetSelectedItemColumnText(girllist_id, GSelection, ss.str(), 5);
						ss.str("");
						ss << g_Arena.m_JobManager.JobName[(int)selected_girl->m_NightJob];
