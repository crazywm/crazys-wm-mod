...
bool cJobManager::WorkFarmVeterinarian(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	string message = "";
	...
	int jobperformance = (	g_Girls.GetSkill(girl, SKILL_BEASTIALITY) + 
	...
	if (g_Girls.HasTrait(girl, "Psychic"))
		jobperformance += 10;
	if (g_Girls.HasTrait(girl, "Optimist"))//less willing to give up on the animal
		jobperformance += 10;
	...
	if (g_Girls.HasTrait(girl, "Meek"))
		jobperformance -= 20;
	if (g_Girls.HasTrait(girl, "Pessimist"))//more likely to give up on the animal
		jobperformance -= 10;
