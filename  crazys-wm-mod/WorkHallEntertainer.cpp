...
bool cJobManager::WorkHallEntertainer(sGirl* girl, sBrothel* brothel, bool Day0Night1, string& summary)
{
	...
	sGirl* dealeronduty = NULL;
	string dealername = "the Dealer";	// Who?
	vector<sGirl *> dealer = g_Brothels.GirlsOnJob(0, JOB_DEALER, Day0Night1);
	if (dealer.size() > 0) dealeronduty = dealer[g_Dice%dealer.size()];
	if (dealeronduty)	dealername = "Dealer " + dealeronduty->m_Realname + "";
	else dealername = "";	// no dealer
	...
	if (g_Dice.percent(50))
	{
		...
		{
			message += " " + girlName + "'s cheeriness improved her performance. ";
			jobperformance += 5;
		}
		if (g_Dice.percent(10))
		{
			...
			{
				message += " A born stripper, " + girlName + " wears her clothes just short of showing flesh. The customers like this. ";
				jobperformance += 15;
			}
			if (g_Girls.GetStat(girl, STAT_PCHATE) > g_Girls.GetStat(girl, STAT_PCFEAR))
			{
				message += " " + girlName + " opened with some rather rude jokes about you. While this annoys you a little, ";
				if (g_Girls.HasTrait(girl, "Your Daughter"))
				{
					message += "she is your daughter, and ";
				}
				message += "it seems to get the audience on her side. ";
				jobperformance += 15;
			}
		}
		message += "\n";
	...
	else if (jobperformance >= 185)
	{
		...
		else if (roll <= 40)
		{
			if (g_Girls.HasTrait(girl, "Aggressive"))
			{
				message += "The savagery of " + girlName + "'s temper has honed her wit to a razor's edge, letting her jab away at the audience and still rake in the tips.\n";
			}
	...
	else if (jobperformance >= 100)
	{
		...
		else if (roll <= 40)
		{
			if (g_Girls.HasTrait(girl, "Charming") || g_Girls.HasTrait(girl, "Charismatic"))
			{
				message += "The tips she recieved were far more than " + girlName + "'s skills had any reason to bring in.\n";
			}
			else
			{
	...
	else
	{
		...
		else if (roll <= 80)
		{
			if (g_Girls.HasTrait(girl, "Aggressive"))
			{
				message += "When the crowd booed her, " + girlName + " responded by throwing a shoe at one of the loudest and most insulting members.\n";
			}
			else
			{
		...
		else
		{
			...
			{
				message += "For some reason, " + girlName + " attempted a dramatic monologue and feigned suicide on stage.  Needless to say, she didn't receive any tips.\n";
			}
		}
	}
	...
	{
		if (jobperformance < 125)
		{ message += girlName + " tried to distract the patrons but due to her lack of skills she distracted " + dealername + " causing you to lose some money.\n"; wages -= 10; }
		else
		{ message += girlName + " was able to perfectly distract some patrons while the " + dealername + " cheated to make some more money.\n"; wages += 25; }
	}
	...
	if (roll <= 5)
	{ message += "\nSeveral patrons heckled her and made her shift generally unpleasant."; work -= 1; }
	else if (roll <= 25)
	{ message += "\nShe had a great time working."; work += 3; }
	else
	{ message += "\nOtherwise, the shift passed uneventfully."; work += 1; }

	g_Girls.UpdateEnjoyment(girl, ACTION_WORKHALL, work, true);
	girl->m_Events.AddMessage(message, IMGTYPE_BUNNY, Day0Night1);
