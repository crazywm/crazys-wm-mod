...
void cObjectiveManager::LoadFromXML(TiXmlElement& root)
{
    TiXmlElement* pObjective = root.FirstChildElement("Objective");
    if (pObjective)
    {
        m_Objective.reset(new sObjective());
        pObjective->QueryIntAttribute("Difficulty", &m_Objective->m_Difficulty);
        pObjective->QueryIntAttribute("Limit", &m_Objective->m_Limit);
        pObjective->QueryValueAttribute<unsigned>("Objective", &m_Objective->m_Objective);
...
void cObjectiveManager::SaveToXML(TiXmlElement& root) const
{
    if (m_Objective)
    {
        TiXmlElement* pObjective = new TiXmlElement("Objective");
        root.LinkEndChild(pObjective);
