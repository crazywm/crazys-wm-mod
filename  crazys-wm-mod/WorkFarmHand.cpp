...
bool cJobManager::WorkFarmHand(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	...
	message = girlName;	
	message += gettext(" worked cleaning the farm.\n\n");

	
	int roll = g_Dice%100;
	if (roll <= 10 && g_Girls.DisobeyCheck(girl, ACTION_WORKCLEANING, brothel))
	{
		message = girl->m_Realname + gettext(" refused to clean the farm.");
		girl->m_Events.AddMessage(message, IMGTYPE_PROFILE, EVENT_NOWORK);
		return true;
	}
	else if(roll <= 15) {
		message += gettext(" She did not like cleaning the farm today.\n\n");
