...
IBuildingScreenManagement::IBuildingScreenManagement(BuildingType type, const char * base_file) :
        cInterfaceWindowXML(base_file), m_Type(type)
{
}
...
void IBuildingScreenManagement::on_select_job(int selection)
{
    ...
    if (selection != -1)
    {
        EditTextItem(job_manager().JobDesc[selection], jobdesc_id);		// first handle the descriptions
        int pos = 0;
...
void IBuildingScreenManagement::assign_job(sGirl * girl, int new_job, int girl_selection, bool fulltime)
{
    ...
    ss.str("");
    ss << job_manager().JobName[night_job];
    ...
    {
        ss.str(""); ss << job_manager().JobName[day_job] << " (" << girl->m_WorkingDay << "%)*" << (interrupted ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "DayJob");
    }
    else if (day_job == JOB_GETABORT)	// `J` added
    {
        ss.str(""); ss << job_manager().JobName[day_job] << " (" << 2 - girl->m_WorkingDay << ")*" << (interrupted ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "DayJob");
    }
    else if (cJobManager::is_Surgery_Job(day_job))	// `J` added
    {
        ss.str(""); ss << job_manager().JobName[day_job] << " (" << 5 - girl->m_WorkingDay << ")*" << (interrupted ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "DayJob");
    }
    else if (interrupted)
    {
    ...
    {
        ss.str(""); ss << job_manager().JobName[night_job] << " (" << girl->m_WorkingDay << "%)*" << (interrupted ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "DayJob");
    }
    else if (night_job == JOB_GETABORT)	// `J` added
    {
        ss.str(""); ss << job_manager().JobName[night_job] << " (" << 2 - girl->m_WorkingDay << ")*" << (interrupted ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "NightJob");
    }
    else if (cJobManager::is_Surgery_Job(night_job))	// `J` added
    {
        ss.str(""); ss << job_manager().JobName[night_job] << " (" << 5 - girl->m_WorkingDay << ")*" << (interrupted ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "NightJob");
    }
    else if (interrupted)
    {
    ...
    {	// `J` added
        ...
        else
        {
            ...
            ss << job_manager().JobName[girl->m_YesterDayJob] << ", if you send her somewhere else, she will have to start her Surgery over.";
        }
        EditTextItem(ss.str(), jobdesc_id);
    }
    ...
    {
        ss.str("");	ss << job_manager().JobName[day_job] << " (" << girl->m_WorkingDay << "%)";
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "DayJob");
    }
    ...
    {
        ss.str("");	ss << job_manager().JobName[night_job] << " (" << girl->m_WorkingDay << "%)";
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "NightJob");
    }
    else if ((girl->m_WorkingDay > 0 || girl->m_PrevWorkingDay > 0) && (
    ...
    {	// `J` added
        ss.str("");	ss << job_manager().JobDesc[new_job] <<
                          "\n** This girl was in training for " << job_manager().JobName[girl->m_YesterDayJob]
    ...
    {
        ss.str(""); ss << job_manager().JobName[night_job] << (CrewNeeded(*girl->m_Building) ? " **" : "");
        SetSelectedItemColumnText(girllist_id, girl_selection, ss.str(), "NightJob");
    }
}
...
bool IBuildingScreenManagement::is_job_allowed(JOBS job)
{
    ...
    // special cases
    if (selected_girl && selected_girl->has_trait("AIDS") && (job == JOB_DOCTOR || job == JOB_NURSE || job == JOB_INTERN))
        return false;
...
{
    return job_manager().JobFilterDesc[selection];
}
...
{
    stringstream jdmessage; jdmessage << job_manager().JobFilterDesc[selection];
