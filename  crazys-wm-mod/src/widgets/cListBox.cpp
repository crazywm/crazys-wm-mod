...
void cListBox::DrawWidget(const CGraphics& gfx)
{
	...
	if (m_ShowHeaders)
	{
		...
		// blit to the screen
        gfx.BlitSurface(m_HeaderBackground, nullptr, &offset);
		...
		for (int i = 0; i<m_ColumnCount; i++)
		{
			...
			if (!m_SortedColumn.empty() && m_SortedColumn == m_ColumnName[i])
			{
				...
				SDL_BlitSurface(m_HeaderSortBack, nullptr, gfx.GetScreen(), &sort_offset);
			}
    ...
    for (auto item = m_Items.begin(); item != m_Items.end(); ++item, ++pos)
    {
		...
		// blit to the screen
        gfx.BlitSurface(m_ElementBorder, nullptr, &offset);
		...
		if (item->m_Selected)
		{
			switch (item->m_Color)
			{
			case 1:		gfx.BlitSurface(m_SelectedRedBackground, nullptr, &offset);;		break;
			case 2:		gfx.BlitSurface(m_SelectedDarkBlueBackground, nullptr, &offset);;	break;
			case 3:		gfx.BlitSurface(m_SelectedGreenBackground, nullptr, &offset);;		break;	// `J` added green
			case 4:		gfx.BlitSurface(m_SelectedYellowBackground, nullptr, &offset);;	break;	// `J` added yellow
			default:	gfx.BlitSurface(m_ElementSelectedBackground, nullptr, &offset);;	break;
			}
		}
		else
		{
			switch (item->m_Color)
			{
			case 1:		gfx.BlitSurface(m_RedBackground, nullptr, &offset);;				break;
			case 2:		gfx.BlitSurface(m_DarkBlueBackground, nullptr, &offset);;			break;
			case 3:		gfx.BlitSurface(m_GreenBackground, nullptr, &offset);;				break;	// `J` added green
			case 4:		gfx.BlitSurface(m_YellowBackground, nullptr, &offset);;			break;	// `J` added yellow
			default:	gfx.BlitSurface(m_ElementBackground, nullptr, &offset);;			break;
			}
		}
...
void cListBox::DefineColumns(string name[], string header[], int offset[], bool skip[], int columns)
{
	...
	for (int i = 0; i<m_ColumnCount; i++)
	{
		...
		m_Header[i] = header[i];
		m_ColumnOffset[i] = int(offset[i] * g_Graphics.GetScaleX());
...
void cListBox::OnKeyPress(SDL_keysym key)
{
