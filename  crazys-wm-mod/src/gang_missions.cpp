...
bool IGangMission::run(sGang& gang)
{
    if (cfg.debug.log_debug()) {
        g_LogFile.ss() << "Debug: Running mission '" << m_Name << "' for gang '" << gang.name() << "'"; g_LogFile.ssend();
    }
...
bool cMissionGrandTheft::execute_mission(sGang& gang, std::stringstream& ss)
{
    ...
    if (defenders)
    {
        ...
        if (!gang_manager().GangBrawl(&gang, defenders.get()))
        {
...
bool cMissionKidnap::kidnap(sGang& gang, std::stringstream& ss, sGirl& girl)
{
    ...
    {
        ss << "try to catch her in their net" << (gang.num_nets() > 1 ? "s" : "") << ".\n";
        int max_tries = gang.num_nets();
        for(int i = 0; i < max_tries && !captured; ++i)
        {
            captured = gang.use_net(girl);
        }
        if (captured)
        {
            int damagednets = start_nets - gang.num_nets();
            if (damagednets > 0)
            {
                ss << girlName << " managed to damage " << damagednets << " of their nets before they finally caught her in ";
                if (gang.num_nets() == 0)      ss << "the tattered remains of their last net.\n";
                else if (gang.num_nets() == 1) ss << "their last net.\n";
                else ss << "their nets.\n";
            }
            ...
            NGmsg << girlName << " was captured in a net and dragged back to the dungeon by " << gang.name() << ".";
    ...
    if (!captured)
    {
        if (girl.fights_back())	// kidnap her
        {
            ss << "attempt to kidnap her.\n";
            if (!gang_manager().GangCombat(&girl, &gang))
            {
                girlimagetype = IMGTYPE_DEATH;
                dungeonreason = DUNGEON_GIRLKIDNAPPED;
                girl.m_Stats[STAT_OBEDIENCE] = 0;
                ...
                NGmsg << girl.m_Realname << " fought with " << gang.name() << " but lost. She was dragged back to the dungeon.";
        ...
        else
        {
            ...
            NGmsg << girl.m_Realname << " was surrounded by " << gang.name() << " and gave up without any more of a fight.";
            captured = true;
        }
    }
...
bool cMissionSabotage::execute_mission(sGang& gang, std::stringstream& ss)
{
    ...
    */
    ...
    if (rival->m_NumGangs > 0)
    {
        ...
        if (!gang_manager().GangBrawl(&gang, &rival_gang))
        {
            if (gang.m_Num > 0)
            {
...
bool cMissionRecapture::execute_mission(sGang& gang, std::stringstream& event_text)
{
    ...
    if (!runaway->fights_back())
    {
        ss << "she comes quietly without putting up a fight.";
        RGmsg << girlName << " was recaptured by " << gang.name() << ". She gave up without a fight.";
        captured = true;
    }
    if (!captured && gang.num_nets() > 0)	// try to capture using net
    {
        ss << "they try to catch her in their net" << (gang.num_nets() > 1 ? "s" : "") << ".\n";
        int max_tries = gang.num_nets();
        for(int i = 0; i < max_tries && !captured; ++i)
        {
            captured = gang.use_net(*runaway);
        }

        damagednets = start_nets - gang.num_nets();
        ...
        if (captured)
        {
            ...
            ss << "She struggles against the net your men use, but it is pointless.  She is in your dungeon now.\n";
            girlimagetype = IMGTYPE_DEATH;
            RGmsg << girlName << " was captured in a net and dragged back to the dungeon by " << gang.name() << ".";
            gang.BoostStat(STAT_INTELLIGENCE, 2);
        }
        else
        {
    ...
    if (!captured)
    {
        if (runaway->fights_back())	// kidnap her
        {
            if (damagednets == 0) ss << "attempt to recapture her.\n";
            if (!gang_manager().GangCombat(runaway, &gang))
            {
                girlimagetype = IMGTYPE_DEATH;
        ...
        else if (damagednets == 0)
        {
            ...
            RGmsg << girlName << " was surrounded by " << gang.name() << " and gave up without a fight.";
            captured = true;
        }
        else
        {
            ss << "After dodging all of their nets, she gives up when they pull out their weapons and prepare to kill her.";
            RGmsg << girlName << " was surrounded by " << gang.name() << " and gave up without anymore of a fight.";
            captured = true;
        }
    }
...
bool cMissionPettyTheft::execute_mission(sGang& gang, std::stringstream& ss)
{
    ...
    if (g_Dice.percent(chance))
    {
        ...
        rival_gang.give_potions(10);
        if (gang_manager().GangBrawl(&gang, &rival_gang)) ss << "Your men win.";
        else
        {
...
bool cMissionCatacombs::execute_mission(sGang& gang, std::stringstream& ss)
{
    ...
    if (!gang_manager().Control_Gangs())	// use old code
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            if (gang.num_potions() == 0) { gang.m_Num--; continue; }
    ...
    else	// use new code
    {
        ...
        while (gang.m_Num >= 1 && bringbacknum < gang.m_Num * max(1, gang.strength() / 20))
        {
            ...
            if (choice < gang_manager().Gang_Gets_Girls())					// get girl = 10 point
            {
                ...
                {
                    int max_tries = gang.num_nets();
                    for(int i = 0; i < max_tries && !gotgirl; ++i)
                    {
                        gotgirl = gang.use_net(*tempgirl);
                    }
                }
                if (!gotgirl)	// fight the girl if not netted
                {
                    if (!gang_manager().GangCombat(tempgirl, &gang)) gotgirl = true;
                }
                delete tempgirl;
                if (gotgirl)
                {
