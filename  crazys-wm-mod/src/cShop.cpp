...
bool is_warrior(const sGirl& girl) {
    return girl.has_trait( "Adventurer") || girl.has_trait( "Assassin") ||
        girl.combat() >= 50 || girl.magic() >= 50;
}

bool cShop::GirlWantsItem(const sGirl& girl, const sInventoryItem& item) const {
    if (girl.has_item(item) > -1	        // if she already has one
...
bool cShop::GirlBuyItem(sGirl& girl, const sInventoryItem& item) {
    ...
    // TODO fix constness
    int temp = girl.add_inv(const_cast<sInventoryItem*>(&item));
    if (temp != -1 && AutoEquip) girl.equip(temp, false);	// MYR: Check temp value
    return true;
