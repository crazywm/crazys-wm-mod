                         ...
                         bool daughter, bool isdaughter, string findbyname)
{
	...
	{
        newGirl->set_stat(i, g_Dice.in_range(girl_template.m_MinStats[i], girl_template.m_MaxStats[i]));
	}
	...
	{
		newGirl->add_temporary_trait("Kidnapped", max(5, g_Dice.bell(0, 25)));		// 5-25 turn temp trait
		int spirit = g_Dice.bell(50, 125);
		int conf = g_Dice.bell(50, 125);
		int obey = g_Dice.bell(-50, 50);
		int hate = g_Dice.bell(0, 100);

		// TODO these bounds should be implemented in set_stat
		newGirl->set_stat(STAT_SPIRIT, min(100, spirit));
		newGirl->set_stat(STAT_CONFIDENCE, min(100, conf));
		newGirl->set_stat(STAT_OBEDIENCE, max(0, obey));
	...
	if (daughter || newGirl->is_yourdaughter())	// `J` if she is your daughter...
	{
		...
		newGirl->set_stat(STAT_OBEDIENCE, max(newGirl->obedience(), 80));	// She starts out obedient
		daughter = true;
	}
	if (isdaughter || newGirl->has_status(STATUS_ISDAUGHTER))
	{
...
{
	cJobManager& m_JobManager = g_Game->job_manager();
