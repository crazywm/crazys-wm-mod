...
bool WorkSOStraight(sGirl& girl, bool Day0Night1, cRng& rng)
{
...
#pragma region //	Job setup				//
	...
	ss << "You procede to change ${name}'s sexual orientation to Straight.\n \n";
	...
	if (girl.has_active_trait("Succubus")) girl.m_WorkingDay += rng % 15;			// Males are easier to drain energy from
	if (girl.has_active_trait("Your Wife")) girl.m_WorkingDay += rng % 10;			// She wants to be with you only
	if (girl.has_active_trait("Your Daughter")) girl.m_WorkingDay += rng.bell(-1, 5);	// She wants to be "Normal"
	if (girl.has_active_trait("Virgin"))		girl.m_WorkingDay += rng.bell(-2, 2);	// Having never been with a man she is a little scared
	// Negative Traits
	...
	if (girl.has_active_trait("Shape Shifter")) girl.m_WorkingDay -= rng % 10;			// If she can become anyone can she really be straight?
	if (girl.has_active_trait("Iron Will")) girl.m_WorkingDay -= rng % 5;			// She is set in her ways
	if (girl.has_active_trait("Futanari")) girl.m_WorkingDay -= rng % 5;			// If she has a dick can she really be straight?
	if (girl.has_active_trait("Porn Star")) girl.m_WorkingDay -= rng % 2;			// She is used to having sex with anyone her director tells her to
	if (girl.has_active_trait("Whore")) girl.m_WorkingDay -= rng % 2;			// She'll do anyone as long as they can pay
	if (girl.has_active_trait("Slut")) girl.m_WorkingDay -= rng % 2;			// She'll do anyone
	if (girl.has_active_trait("Actress")) girl.m_WorkingDay -= rng % 2;			// She will do whatever her director tells her to
...
double JP_SOStraight(const sGirl& girl, bool estimate)// not used
{
	if (girl.has_active_trait("Straight"))	return -1000;
	return 250;
}
