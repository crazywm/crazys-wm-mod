    ...
    while (girl->health() > 20 && pHealth > 0 && combatrounds < 1000)
    {
        ...
        if (g_Dice.percent(girl->get_skill(attack)))
        {
            ...
            if (!g_Dice.percent(pdodge))
                ...
                agility(g_Dice % 2);	// player may improve a little
        }

        // Player Attacks
        if (g_Dice.percent(get_skill(pattack)))
        {
            int damage = 0;
            ...
            else
            {
                ...
                damage = 5 + get_skill(pattack) / 10;
            }

            upd_skill(pattack, g_Dice % 2);  // he may improve a little
