...
public:
    ...
    // xml helpers
    void save_girls_xml(TiXmlElement& target) const;
    void save_settings_xml(TiXmlElement& root) const;
    virtual void save_additional_xml(TiXmlElement& root) const {};
    void load_girls_xml(TiXmlHandle root);
    void load_settings_xml(TiXmlElement& root);

    virtual void load_xml(TiXmlElement& root);
    virtual void save_xml(TiXmlElement& root) const;
    ...
    bool is_sex_type_allowed(unsigned int sex_type) const;
...
private:
    ...
    bool m_RestrictOral    = false;
    bool m_RestrictTitty   = false;
    bool m_RestrictHand    = false;
    bool m_RestrictBeast   = false;
    bool m_RestrictGroup   = false;
    bool m_RestrictNormal  = false;
    bool m_RestrictLesbian = false;
    bool m_RestrictFoot    = false;
    bool m_RestrictStrip   = false;
...
inline bool is_nonhuman(const sGirl* girl) {
    return girl->has_trait("Not Human");
}
...
struct HasJob {
    ...
    bool operator()(const sGirl* girl) const {
        if (m_DayNight)
        {
            return girl->m_NightJob == m_Job;
        }
        else
        {
            return girl->m_DayJob == m_Job;
        }
    }
