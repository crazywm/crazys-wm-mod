...
bool cJobManager::WorkWhore(sGirl* girl, bool Day0Night1, string& summary)
{
	...
	*/
	...
	if (!bStreetWork && LoopCount >g_Game.GetNumCustomers()) LoopCount = g_Game.GetNumCustomers();
	...
	for (int i = 0; i < LoopCount; i++)	// Go through all customers
	{
		...
		else if (!bStreetWork && !brothel->is_sex_type_allowed(Cust.m_SexPref)) // it their first sexpref is banned then switch to the second
		{
