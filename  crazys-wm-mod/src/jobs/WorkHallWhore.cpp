...
// `J` Job Brothel - Hall
bool cJobManager::WorkHallWhore(sGirl* girl, bool Day0Night1, string& summary)
{
	...
	*/
	...
	for (int i = 0; i < LoopCount; i++)	// Go through all customers
	{
		...
		if (!brothel->is_sex_type_allowed(Cust.m_SexPref) && !brothel->is_sex_type_allowed(Cust.m_SexPrefB))
		{
			brothel->m_RejectCustomersRestrict++;
			continue;	// `J` if both their sexprefs are banned then they leave
		}
		else if (!brothel->is_sex_type_allowed(Cust.m_SexPref)) // it their first sexpref is banned then switch to
		    // the second
		{
		...
		{
            CatchGirl(girl, fuckMessage, guardgang);
        }
		else
		{
