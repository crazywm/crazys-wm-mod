...
bool cJobManager::WorkFilmBuk(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
	...
	else if (roll <= 10 && !girl->has_trait( "Mind Fucked") && girl->disobey_check(ACTION_WORKMOVIE, JOB_FILMBUKKAKE))
	{
	...
	cGirls::PossiblyGainNewTrait(girl, "Masochist", 75, ACTION_SEX, girlName + " has turned into a Masochist from all the degradation.", Day0Night1);
	cGirls::PossiblyGainNewTrait(girl, "Cum Addict", 80, ACTION_WORKMOVIE, girlName + " has become addicted to cum.", Day0Night1);
	if (tied)
	{
		cGirls::PossiblyGainNewTrait(girl, "Mind Fucked", 75, ACTION_SEX, girlName + " has become Mind Fucked from the forced degradation.", Day0Night1);
	}
	//lose
	if (tied) cGirls::PossiblyLoseExistingTrait(girl, "Iron Will", 80, ACTION_SEX, girlName + "'s unwilling degradation has shattered her iron will.", Day0Night1);
