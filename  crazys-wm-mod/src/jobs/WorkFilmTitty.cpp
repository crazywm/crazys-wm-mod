...
bool cJobManager::WorkFilmTitty(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
	...
	int roll = rng.d100();
	if (roll <= 10 && girl->disobey_check(ACTION_WORKMOVIE, JOB_FILMTITTY))
	{
		ss << "She refused to have her titty's fucked on film today.\n";
		girl->m_Events.AddMessage(ss.str(), IMGTYPE_PROFILE, EVENT_NOWORK);
		return true;
	}
	else if (roll <= 10) { enjoy -= rng % 3 + 1;	ss << "She used her breasts on his cock, but didn't like it.\n \n"; }
	else if (roll >= 90) { enjoy += rng % 3 + 1;	ss << "She loved having his cock slide between her breasts.\n \n"; }
	else /*            */{ enjoy += rng % 2;		ss << "She had a pleasant day using her tits to get her co-star off.\n \n"; }
	jobperformance = enjoy * 2;

	// remaining modifiers are in the AddScene function --PP
	int finalqual = brothel->AddScene(girl, JOB_FILMTITTY, bonus);
