...
// `J` Job Farm - Staff
bool cJobManager::WorkFarmMarketer(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
...
#pragma region	//	Create Items				//
	...
	if (ForSale_Food >= 10000 && rng.percent(5))
	{
		sGirl* ugirl = nullptr;
		int cost = 10000;
		if (ForSale_Food >= 15000 && rng.percent(cfg.slavemarket.unique_market()))
		{
			cost = 15000;
		...
		{
			cost = 10000;
			ugirl = g_Game->CreateRandomGirl(0, false);	// create a random girl
		}
		if (ugirl)
		{
			stringstream Umsg;
			...
			ugirl->constitution(30 - rng % 40);		ugirl->intelligence(10 - rng % 15);
			ugirl->confidence(10 - rng % 30);		ugirl->agility(20 - rng % 25);
			ugirl->strength(1 + rng % 20);			ugirl->obedience(10 + rng % 40);
			ugirl->spirit(50 - rng % 100);			ugirl->morality(10 + rng % 40);
			ugirl->refinement(30 - rng % 40);		ugirl->dignity(20 - rng % 40);
			ugirl->npclove(rng.bell(-100, 100));		// she may have had a boyfriend before she got sold to you
			ugirl->service(10 + rng % 30);			ugirl->performance(rng % 5);
			ugirl->crafting(rng % 10);				ugirl->herbalism(rng % 5);
			ugirl->farming(rng % 10);				ugirl->brewing(rng % 5);
			ugirl->animalhandling(rng % 10);			ugirl->cooking(rng % 20);
...
#pragma region	//	Money					//
	...
	gold += (pricemultiplier * (rng % 141 + 10) * Sell_Alchemy);
...
#pragma region	//	Finish the shift			//
	...
	cGirls::PossiblyGainNewTrait(girl,		"Charismatic",	30, actiontype, girlName + " has been selling long enough that she has learned to be more Charismatic.", Day0Night1);
	cGirls::PossiblyLoseExistingTrait(girl, "Meek",			40, actiontype, girlName + "'s having to work with customers every day has forced her to get over her meekness.", Day0Night1);
	cGirls::PossiblyLoseExistingTrait(girl, "Shy",			50, actiontype, girlName + " has been selling for so long now that her confidence is super high and she is no longer Shy.", Day0Night1);
	cGirls::PossiblyLoseExistingTrait(girl, "Nervous",		70, actiontype, girlName + " seems to finally be getting over her shyness. She's not always so Nervous anymore.", Day0Night1);
	cGirls::PossiblyGainNewTrait(girl,		"Psychic",		90, actiontype, girlName + " has learned to size up the buyers so well that you'd almost think she was Psychic.", Day0Night1);
...
double cJobManager::JP_FarmMarketer(sGirl* girl, bool estimate)// not used
{
    ...
    jobperformance += girl->get_trait_modifier("work.farmmarketer");
