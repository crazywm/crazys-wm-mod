...
bool cJobManager::WorkJanitor(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
	...
	if (playtime)	// `J` needs more variation
	{
		...
		else if (girl->is_pregnant() || rng.percent(40))
		{
			ss << "hung out in the maternity ward watching the babies.";
			girl->happiness((rng % 6) - 2);
		}
		else if (girl->tiredness() > 50 && brothel->free_rooms() > 10)
		{
			ss << "found an empty room and took a nap.";
			girl->tiredness(-((rng % 10) + 5));
		}
		else
		{
			int d = brothel->num_girls_on_job(JOB_DOCTOR, Day0Night1);
			int n = brothel->num_girls_on_job(JOB_NURSE, Day0Night1);
			ss << "watched the ";
			if (d + n < 1)
			{
