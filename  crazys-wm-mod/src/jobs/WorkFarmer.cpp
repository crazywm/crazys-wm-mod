...
// `J` Job Farm - Laborers
bool cJobManager::WorkFarmer(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
...
#pragma region	//	Enjoyment and Tiredness		//
	...
	if (roll_a <= 10)
	{
		...
		if (roll_b < 20)	// injury
		{
			...
			{
				ss << " killing her.";
				g_Game->push_message(girlName + " was killed in an accident at the Farm.", COLOR_RED);
...
#pragma region	//	Create Items				//
	...
	if (rng.percent((girl->farming() + girl->magic()) / 10) && rng.percent(jobperformance / 10))
	{
		...
		if (item)
		{
            g_Game->player().inventory().add_item(item, itemnumber);
...
#pragma region	//	Finish the shift			//
	...
	// Gain Traits
	cGirls::PossiblyGainNewTrait(girl, "Tough", 50, actiontype, "Working in the heat of the sun has made " + girlName + " rather Tough.", Day0Night1);
