...
bool cJobManager::WorkWhore(sGirl* girl, bool Day0Night1, string& summary, cRng& rng) {
    /*
	...
	*/
    ...
    for (int i = 0; i < LoopCount; i++)	// Go through all customers
    {
        ...
        if (!bStreetWork && !brothel->is_sex_type_allowed(Cust.m_SexPref)) // it their first sexpref is banned then switch to the second
        {
            ...
            Cust.m_Stats[STAT_HAPPINESS] = 32 + rng % 9 + rng % 9;	// `J` and they are less happy
        }
        else	// `J` otherwise they are happy with their first choice.
        {
            // WD:	Set the customers begining happiness/satisfaction
            Cust.m_Stats[STAT_HAPPINESS] = 42 + rng % 10 + rng % 10; // WD: average 51 range 42 to 60
            SexType = (SKILLS)Cust.m_SexPref;
        }
        ...
        if (!bCustCanPay && !rng.percent((50 - Cust.m_Stats[STAT_INTELLIGENCE]) / 5))
        {
            //continue;
        ...
        else if (girl->has_trait("Zombie") && Cust.m_Fetish == FETISH_FREAKYGIRLS && rng.percent(10))
        {
        ...
        if (!acceptsGirl)
        {
            ...
            else if (girl->has_trait("Lesbian") && Cust.m_IsWoman && rng.percent(50))
            {
                fuckMessage << "The female customer chooses her because she is a Lesbian.\n \n";
                acceptsGirl = true;
            }
            else if (girl->has_trait("Straight") && Cust.m_IsWoman && rng.percent(10))
            {
                fuckMessage << girlName << " refuses to accept a female customer because she is Straight.\n \n";
                brothel->m_Fame -= 2;
                acceptsGirl = false;
            }
            else if (girl->has_trait("Lesbian") && !Cust.m_IsWoman && rng.percent(10))
            {
            ...
            else if ((girl->has_trait("Queen") || girl->has_trait("Princess")) && Cust.m_SexPref == SKILL_BEASTIALITY && rng.percent(20))
            {
            ...
            else if (rng.percent(20) && girl->has_trait("Social Drinker"))
            {
                fuckMessage << "The customer chooses her because she's fun, flirty and half-cut.\n \n";
                acceptsGirl = true;
            }
            else if (rng.percent(40) && girl->has_trait("Exhibitionist") && girl->beauty() >= 50)
            {
            ...
            else if (rng.percent(5) && (girl->has_trait("Slut") || girl->dignity() >= 70))
            {
        ...
        if (!bCustCanPay)
        {
            if (rng.percent(Cust.m_Stats[STAT_CONFIDENCE] - 25))	// Runner
            {
                fuckMessage << " The customer couldn't pay and ";
                if (guardgang)
                {
                    if (rng%Cust.m_Stats[STAT_AGILITY] > guardgang->m_Num + rng%guardgang->agility())
                    {
        ...
        else if (rng.percent((40 - Cust.m_Stats[STAT_HAPPINESS]) / 2) && rng.percent(Cust.m_Stats[STAT_CONFIDENCE] - 25))
        {
        ...
        else if ((knowwife || knowdaughter) && rng.percent(Cust.m_Stats[STAT_CONFIDENCE] / 5))
        {
        ...
        else  // Customer has enough money
        {
            ...
            if (girl->has_trait( "Your Daughter") && knowdaughter && Cust.m_Money >= 20 && rng.percent(50))
            {
            ...
            else if (girl->has_trait( "Your Wife") && knowwife && Cust.m_Money >= 20 && rng.percent(50))
            {
            ...
            else if (girl->has_trait( "Your Daughter") && Cust.m_Money >= 20 && rng.percent(15))
            {
                if (rng.percent(50))
                {
            ...
            else if (girl->has_trait( "Your Wife") && Cust.m_Money >= 20 && rng.percent(15))
            {
                if (rng.percent(50))
                {
    ...
    //gain
    cGirls::PossiblyGainNewTrait(girl, "Good Kisser", 50, actiontype, girlName + " has had a lot of practice kissing and as such as become a Good Kisser.", Day0Night1);
    cGirls::PossiblyGainNewTrait(girl, "Slut", 60, actiontype, girlName + " has turned into quite a slut.", Day0Night1, EVENT_WARNING);
    cGirls::PossiblyGainNewTrait(girl, "Nymphomaniac", 70, actiontype, girlName + " has been having so much sex she is now wanting sex all the time.", Day0Night1);

    //SIN: use a few of the new traits
    if (rng.percent(1) && rng.percent(girl->oralsex()) && (girl->has_trait( "Nymphomaniac")))
        cGirls::PossiblyGainNewTrait(girl, "Cum Addict", 90, actiontype, girlName + " has tasted so much cum she now craves it at all times.", Day0Night1);
