...
bool cJobManager::WorkSOLesbian(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
...
#pragma region //	Job setup				//
	...
	// Positive Traits
	if (girl->check_virginity())		girl->m_WorkingDay += rng.bell(2, 10);	// If she has not been with a man, what could she be missing
	if (girl->has_trait("Your Daughter"))	girl->m_WorkingDay += rng.bell(2, 10);	// She sees you enjoying yourself with the ladies, lead by example?
	...
	if (girl->has_trait("Succubus"))		girl->m_WorkingDay -= rng % 10;			// Males are easier to drain energy from
	if (girl->has_trait("Your Wife"))		girl->m_WorkingDay -= rng % 10;			// She wants to be with you (this will changed when player gender is added)
	if (girl->has_trait("Shape Shifter"))	girl->m_WorkingDay -= rng % 10;			// If she can become anyone can she really be a lesbian?
	if (girl->has_trait("Whore"))			girl->m_WorkingDay -= rng % 5;			// She'll do anyone as long as they can pay
	if (girl->has_trait("Slut"))			girl->m_WorkingDay -= rng % 5;			// She'll do anyone
	if (girl->has_trait("Actress"))			girl->m_WorkingDay -= rng % 5;			// She will do whatever her director tells her to
