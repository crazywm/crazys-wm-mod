...
bool cJobManager::WorkSOStraight(sGirl* girl, bool Day0Night1, string& summary)
{
...
#pragma region //	Job setup				//
	...
	if (brothel->is_sex_type_allowed(SKILL_ANAL))			girl->m_WorkingDay -= g_Dice % 10;
	if (brothel->is_sex_type_allowed(SKILL_BDSM))			girl->m_WorkingDay -= g_Dice % 10;
	if (brothel->is_sex_type_allowed(SKILL_BEASTIALITY))	girl->m_WorkingDay += g_Dice % 2;
	if (brothel->is_sex_type_allowed(SKILL_FOOTJOB))		girl->m_WorkingDay -= g_Dice % 3;
	if (brothel->is_sex_type_allowed(SKILL_GROUP))			girl->m_WorkingDay -= g_Dice % 10;
	if (brothel->is_sex_type_allowed(SKILL_HANDJOB))		girl->m_WorkingDay -= g_Dice % 5;
	if (brothel->is_sex_type_allowed(SKILL_LESBIAN))		girl->m_WorkingDay += g_Dice % 10 + 10;
	if (brothel->is_sex_type_allowed(SKILL_NORMALSEX))		girl->m_WorkingDay -= g_Dice % 20 + 10;
	if (brothel->is_sex_type_allowed(SKILL_ORALSEX))		girl->m_WorkingDay -= g_Dice % 10;
	if (brothel->is_sex_type_allowed(SKILL_TITTYSEX))		girl->m_WorkingDay -= g_Dice % 4;
