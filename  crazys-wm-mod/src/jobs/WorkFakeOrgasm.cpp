...
bool cJobManager::WorkFakeOrgasm(sGirl* girl, bool Day0Night1, string& summary, cRng& rng)
{
...
#pragma region //	Job setup				//
	...
	// Positive Traits
	if (girl->has_trait("Quick Learner"))	girl->m_WorkingDay += rng % 20;			//
	if (girl->has_trait("Actress"))			girl->m_WorkingDay += rng % 20;			// Acting is just faking anyway
	if (girl->has_trait("Porn Star"))		girl->m_WorkingDay += rng % 20;			// The director just wants everything to cum together
	if (girl->has_trait("Your Daughter"))	girl->m_WorkingDay += rng % 20;			// Living in a brothel you pick up a few things over the years
	if (girl->has_trait("Your Wife"))		girl->m_WorkingDay += rng % 10;			// Most wives fake it eventually
	if (girl->has_trait("Shape Shifter"))	girl->m_WorkingDay += rng % 10;			// She is used to faking who she is
	if (girl->has_trait("Exhibitionist"))	girl->m_WorkingDay += rng % 5;			// She enjoys making a scene
	if (girl->has_trait("Whore"))			girl->m_WorkingDay += rng % 5;			// Doing it so many times makes faking it a little easier
	if (girl->has_trait("Slut"))			girl->m_WorkingDay += rng % 5;			// Doing it so many times makes faking it a little easier
	if (girl->has_trait("Succubus"))		girl->m_WorkingDay += rng % 5;			// If faking it gets them to cum faster, do it
	if (girl->has_trait("Iron Will"))		girl->m_WorkingDay += rng % 3;			// She is going to finish her taks, whatever it is
	if (girl->has_trait("Audacity"))		girl->m_WorkingDay += rng % 3;			// She doesn't care what it looks like or who sees it
	// Negative Traits
	...
	if (girl->has_trait("Fast Orgasms"))	girl->m_WorkingDay -= rng % 10;			// She cums before she can fake it
	if (girl->has_trait("Futanari"))		girl->m_WorkingDay -= rng % 10;			// Kind of hard to fake an orgasm with a dick
	if (girl->has_trait("Broodmother"))		girl->m_WorkingDay -= rng % 10;			// faking it kind of defeats the purpose
	if (girl->has_trait("Bimbo"))			girl->m_WorkingDay -= rng % 5;			// She enjoys herself to much to remember to fake it
	if (girl->has_trait("Nymphomaniac"))	girl->m_WorkingDay -= rng % 5;			// She enjoys herself to much to remember to fake it
	if (girl->has_trait("Blind"))			girl->m_WorkingDay -= rng % 5;			// She can't see her partner to time it right
	if (girl->has_trait("Deaf"))			girl->m_WorkingDay -= rng % 5;			// She can't hear what she sounds like
	if (girl->has_trait("Botox Treatment"))	girl->m_WorkingDay -= rng % 5;			// Is she faking it? can't tell by her face
...
#pragma region	//	Finish the shift			//
	...
	libido += girl->has_trait("Nymphomaniac") ? rng.bell(3, 10) : rng.bell(1, 5);
