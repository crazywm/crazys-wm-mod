...
#pragma region //	Job Performance			//
	...
	else if (g_Dice.percent(5))  //glory hole event
	{
			...
			<< " saw his cock sticking out and ";
		{
			...
			else if (brothel->is_sex_type_allowed(SKILL_NORMALSEX) && !girl->check_virginity() && (girl->has_trait( "Nymphomaniac") || girl->has_trait( "Succubus")) && girl->libido() >= 80) //sex
			{
				sextype = SKILL_NORMALSEX;
				ss << "decided she needed to use it for her own entertainment.\n";
			}
			else if (brothel->is_sex_type_allowed(SKILL_ORALSEX) && (girl->has_trait( "Nymphomaniac") || girl->has_trait( "Succubus") || girl->has_trait( "Cum Addict")) && girl->libido() >= 60) //oral
			{
				sextype = SKILL_ORALSEX;
				ss << "decided she needed to taste it.\n";
			}
			else if (brothel->is_sex_type_allowed(SKILL_FOOTJOB) && (girl->has_trait( "Nymphomaniac") || girl->has_trait( "Succubus") || girl->dignity() < -30) && girl->libido() >= 40) //foot
			{
				sextype = SKILL_FOOTJOB;
				imagetype = IMGTYPE_FOOT;
				ss << "decided she would give him a foot job for being so brave.\n";
			}
			else if (brothel->is_sex_type_allowed(SKILL_HANDJOB) && (girl->has_trait( "Nymphomaniac") || girl->has_trait( "Succubus") || girl->dignity() < -30))	//hand job
			{
...
#pragma region	//	Finish the shift			//
	...
	//lose
	cGirls::PossiblyLoseExistingTrait(girl, "Nervous", 30, ACTION_WORKSTRIP, girlName + " has had so many people see her naked she is no longer nervous about anything.", Day0Night1);
