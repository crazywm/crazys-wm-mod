...
bool cJobManager::WorkBarWaitress(sGirl* girl, sBrothel* brothel, bool Day0Night1, string& summary)
{
	...
	if(Preprocessing(ACTION_WORKBAR, girl, brothel, Day0Night1, summary, ss.str())) 		return true;
	...
	int wages = 15, work = 0;
	ss << "She worked as a waitress in the bar.\n";
	...
	if (g_Girls.HasTrait(girl, "One Eye"))				  jobperformance -= 10;
	 ...
	 if(jobperformance >= 245)
		...
		if (roll <= 14)
			...
			{
				ss <<  "Knowing how to speak with customers, " + girlName + " always gets the clients to order something extra or more pricy then they wanted at the beginning, making you a tidy profit.\n";
				wages += 10;
			}
		else if (roll <= 42)
			{
	...
	{ ss << "Added a litte something extra to the patrons order to spice it up. They enjoyed it greatly and she recieved some nice tips.\n\n"; wages += 25; }
