...
bool cJobManager::WorkFilmLesbian(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	string message = "";
	...
	girl->m_Pay += 60;
	message = girlName;
	message += (" worked as an actress filming lesbian scenes.\n\n");
	
	int roll = g_Dice%100;

	if(roll <= 10 && g_Girls.DisobeyCheck(girl, ACTION_WORKMOVIE, brothel)) 
	{
		message = girlName + " refused to film a lesbian scene today.\n";
	...
	else if(roll <= 15)
	{
		...
		message += girlName + " didn't like doing it, but but she made the actress come.\n\n";
		jobperformance += -5;
	}
	else if(roll >=90)
	{
		g_Girls.UpdateEnjoyment(girl, ACTION_WORKMOVIE, +3, true);
		message += girlName + " was amazed how good it felt to have her pussy eaten, and gave as good in return.\n\n";
		jobperformance += +5;
	}
	else
	{
	...
	string finalqual = g_Studios.AddScene(girl, SKILL_LESBIAN, jobperformance);
