...
bool cJobManager::WorkFluffer(sGirl* girl, sBrothel* brothel, bool Day0Night1, string& summary)
{
	...
	ss << girlName << " worked as a fluffer.\n\n";

	int roll = g_Dice.d100();
	if (roll <= 10 && g_Girls.DisobeyCheck(girl, ACTION_WORKMOVIE, brothel))
	{
		ss << "She refused to work as a fluffer today";
		girl->m_Events.AddMessage(ss.str(), IMGTYPE_PROFILE, EVENT_NOWORK);
		return true;
	}

	/* */if (roll <= 10) { enjoy -= g_Dice % 3 + 1; ss << "She didn't like having so many dicks in her mouth today.\n\n"; }
	else if (roll >= 90) { enjoy += g_Dice % 3 + 1; ss << "She loved sucking cock today.\n\n"; }
	else /*    */{ enjoy += max(0, g_Dice % 3 - 1); ss << "She had a pleasant day giving blowjobs.\n\n"; }
	...
	{
		wages += jobperformance;
	}

	/* */if (jobperformance > 0)	ss << "She helped improve the scene " << jobperformance << "% by keeping the actors happy with her mouth. \n";
