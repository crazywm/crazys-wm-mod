...
bool cJobManager::WorkFilmOral(sGirl* girl, sBrothel* brothel, int DayNight, string& summary)
{
	string message = "";
	...
	girl->m_Pay += 60;
	message = girlName;
	message += (" worked as an actress filming oral scenes.\n\n");
	
	int roll = g_Dice%100;

	if(roll <= 10 && g_Girls.DisobeyCheck(girl, ACTION_WORKMOVIE, brothel)) 
	{
		message = girlName + " refused to give head on film today.\n";
	...
	else if(roll <= 15)
	{
		...
		message += girlName + " sucked his cock, but didn't like it.\n\n";
		jobperformance += -5;
	}
	else if(roll >=90)
	{
		g_Girls.UpdateEnjoyment(girl, ACTION_WORKMOVIE, +3, true);
		message += girlName + " loved the taste of his cock on her tounge.\n\n";
		jobperformance += +5;
	}
	else
	{
		g_Girls.UpdateEnjoyment(girl, ACTION_WORKMOVIE, +1, true);
		message += girlName + " had a pleasant day sucking her co-star off.\n\n";
	}
	...
	string finalqual = g_Studios.AddScene(girl, SKILL_ORALSEX, jobperformance);
